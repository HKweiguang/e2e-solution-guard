...
**关键设计**：
- **业务产物按模块聚合**（PRD→交互→UI→测试），变更可追溯
- **技术方案按服务/技术边界聚合**，一个服务可支撑多个模块，一个模块可涉及多个服务
- PRD 与技术方案通过「功能点编号」和「技术实现单元」建立多对多映射
- 顶层定义提供跨模块共享的规范体系，模块级产物通过 upstream 表格声明继承关系
- 模块间通过上游文档表格建立引用，确保改动同步

> 详细目录结构示例参见 `references/workflow/product-organization.md`
