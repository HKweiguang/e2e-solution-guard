...
```bash
# 全量审计：编号重复、引用有效性、术语一致性、接口一致性
python3 scripts/doc-audit.py docs/
...
| 交互设计 | **按角色** | 同一角色的交互体验是连贯的，跨模块 |
| UI | **按角色+设备** | 同一角色+设备的视觉风格一致，跨模块 |
| 技术方案 | **按服务/技术边界** | 技术方案的边界是代码的物理边界 |

**PRD ↔ 技术方案是多对多映射**：通过「功能点编号」+「技术实现单元」关联。一个功能可能涉及多个服务，一个服务也可能支撑多个功能。

> 详细目录结构、编号规范、多角色组织方式参见 [`references/workflow/product-organization.md`](references/workflow/product-organization.md)
...
多角色项目：
- **PRD**：一个模块一份，内部按角色分章节；用户故事和用户旅程按角色分表展示
- **交互设计**：按角色分文件
- **UI**：按角色+设备分文件
