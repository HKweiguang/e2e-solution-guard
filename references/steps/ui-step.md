...
| 文档 | 类型 | 引用范围 |
|------|------|---------|
| 交互设计-{角色}-{页面组} | 结构输入 | SVG 线框图 → HTML 结构；状态矩阵 → 状态类/伪类名称映射（行为状态定义，不含视觉值） |
> 注：交互设计按角色分文件（如 `交互设计-c端.md`），同一角色的不同设备（web/app）可合并或拆分。UI 按角色+设备分文件，一个 HTML 只展示一种设备的界面。
...
|---------|-------------|-------------------|
| 页面1 | PAGE-CUST-PROFILE-001 | page-cust-profile-001 |
| 页面2 | PAGE-CUST-ORDER-001 | page-cust-order-001 |
...
**填写示例**（PAGE-CUST-PROFILE-001 个人资料页）：
