...
> 本清单用于指导作者完成 UI 设计时自查，确保产物自身完整。跨步骤对齐（如 UI 与交互设计、技术方案的一致性）在审计阶段由 audit-procedure.md 统一处理，不在此清单中。
...
- [ ] 多端项目时，平台状态差异与项目 UI-顶层定义 §2.12 一致
...
- [ ] 加载态保留了上下文（非全白屏）
- [ ] 产物中无不属于 UI 范畴的行为描述（触发条件、交互流程、页面跳转、状态流转、校验规则）
