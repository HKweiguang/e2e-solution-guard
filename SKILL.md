...
**模块内按角色+设备分文件**：
- **PRD 按用户角色分文件**（C端/B端/管理后台各一份），不同角色的业务需求完全不同
- **交互设计按角色分文件**，同一角色内不同设备（web/App/小程序）可合并或拆分
- **UI 按角色+设备分文件**（一个 HTML 只展示一种设备的界面）
- **技术方案统一**（一套接口支撑多端）
...
├── prd-c端.md                  # C端消费者需求
├── prd-admin.md                # 管理后台需求
├── 交互设计-c端.md              # C端交互（web+app 合并，内部标差异）
...
原则：
- **PRD 按用户角色分**（不同角色的业务目标不同）
- **交互设计按角色分文件**，同一角色内不同设备可合并或拆分
- **UI 按角色+设备分**（不同设备的界面完全不同）
- **技术方案统一**（一套接口支撑多端）
...
| 角色 | 标识 | 功能编号 | 页面编号 |
|------|------|---------|---------|
| C端消费者 | `CUST` | `USER-CUST-001` | `PAGE-CUST-PROFILE-001` |
| B端商家 | `MERCHANT` | `ORDER-MERCHANT-001` | `PAGE-MERCHANT-ORDER-001` |
| 管理后台 | `ADMIN` | `USER-ADMIN-001` | `PAGE-ADMIN-LIST-001` |

### 4.2 产物关联原则

每份产物必须通过文件头部的**上游文档**表格，声明：
1. **引用了哪些上游文档**（如 PRD、顶层定义、交互设计）
2. **引用了上游的哪些内容**（如功能点 `USER-CUST-001`–`USER-CUST-005`、页面 `PAGE-CUST-001`–`PAGE-CUST-003`）
