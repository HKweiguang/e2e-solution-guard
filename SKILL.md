...
**模块内按端分文件**：一个模块如果服务多个端或用户群体（如 C端 + 管理后台），可以在模块目录内按端拆分交互/UI 产物：

```
用户管理/
├── prd.md                    # 统一 PRD，内部按端分节描述
├── 交互设计-c端.md            # C端：注册/登录/个人信息
├── 交互设计-管理后台.md        # 管理后台：用户列表/权限/冻结
├── ui-c端.html
├── ui-管理后台.html
├── 技术方案.md                # 一套接口支撑多端
└── 测试报告.md
...
原则：
- **PRD 和技术方案通常统一**（一套需求/一套接口支撑多端）
- **交互和 UI 按端拆分**（不同端的交互方式、视觉密度差异大）
- 某个端没有独立交互/UI 时（如纯管理后台的数据统计），不产该端的文件
