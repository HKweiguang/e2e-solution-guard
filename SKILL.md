...
description: >
  ...
  (structure + content + semantic depth).
   ...
   checks, semantic audit checklist generation (`--model-checklist`), and full transformation
   analysis (`--transform`), then performs semantic audit by having the AI fill in judgments
   row-by-row (see `references/workflow/audit-procedure.md`).
---
...
| 启动审计 | `references/workflow/audit-procedure.md` | 审计执行策略、语义审计执行步骤、脚本与 AI 分工边界 |
