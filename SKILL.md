...
**模块级视觉实现**：交互设计提供 SVG 页面结构图（结构）+ 状态矩阵（行为），UI 设计产出 HTML 原型（视觉实现）。

步骤顺序由项目根据阶段决定，skill 不强制。每个步骤产出的产物数量（按模块、按步骤或其他方式拆分）由项目需求决定。
...
| 启动审计 | `references/workflow/audit-procedure.md` + `references/steps/{类型}-step.md` | 审计执行策略、语义审计执行步骤、脚本与 AI 分工边界；步骤模板作为审计参照基准；具体检查清单参见 `audit-checklists.md` |
| **改造/重构现有产物** | **`references/workflow/solution-transformation.md`** | **按 skill 标准对现有产物进行结构+内容+语义全量重构，四层防偷懒机制** |
...
| 需要填写示例参考 | `references/examples/*-examples.md` | PRD/交互/UI/技术/测试的填写示例和常见陷阱 |
| 生成/修改后自查 | `references/workflow/audit-procedure.md` §常见遗漏项速查 | 编号连续性、双向引用、术语一致性、顶层定义交叉对齐等硬规则 |
| 执行具体产物类型审计 | `references/workflow/audit-checklists.md` | PRD/交互/UI/技术/测试/代码审计 六类产物的结构化检查清单 |
...
| 用户诉求 | 使用脚本 | 完整路径 | 说明 |
|---------|---------|---------|------|
| 审计**单个**产物文件 | `doc-audit.py` | `$HOME/.agents/skills/e2e-solution-guard/scripts/doc-audit.py` | 单文件机械检查、语义检查点生成 (`--model-checklist`)、改造分析 (`--transform`) |
...
- **对话式主动提问，不一次性抛表格**：AI 不是被动等待用户指令，而是在关键决策点主动发起简短对话（通常 1-3 个问题），像专业顾问一样引导用户澄清需求。触发条件：① 必须确认的信息缺失且影响后续产物；② 用户描述存在多种理解方式；③ 当前决策影响多个下游产物；④ 新需求与已有产物矛盾。提问方式：提供选项让用户选，问题要有上下文锚定（如"这个订单模块涉及消费者和商家两个角色，对吧？还是还有管理后台？"），不是无头问卷。用户说"跳过"→记录最小假设并继续，不阻塞。**禁止**：项目初始化时一次性抛出长问卷（如 10 个问题的表格）；无上下文的问题清单；用户已明确的信息重复确认
- **执行前做 gap 分析，确认后再输出**：加载现有方案和上下文后，先内部识别缺失信息、潜在冲突和范围不明确的地方，再通过对话式提问与用户确认。禁止基于模糊想法直接输出完整方案、产物或代码
...
- **脚本与 AI 分工有边界，不可一条路走到黑**：机械检查（格式、存在性、连续性、编号映射）归 `doc-audit.py`，语义判断（正确性、合理性、满足度）归 AI。**关键防偷懒机制**：`[模型]` 检查项的"穷举列出"也归 `doc-audit.py`（`--model-checklist`），AI 只负责"逐条判断填空"，无权自行决定查几项。扩展检查项时必须先对照 `references/workflow/audit-procedure.md` 中的分工边界与编排边界判断归属，禁止为了让脚本覆盖而降级规则精度，禁止让 AI 做穷举性存在检查，禁止让 AI 自行回忆检查清单
...
| 产物 | 聚合单位 | 说明 |
|------|---------|------|
| PRD、测试报告 | **按业务模块聚合**（推荐） | 同一模块的产物放在一起，便于业务闭环追溯 |
| 交互设计 | **按业务域聚合**（推荐） | 同一业务域的交互体验是连贯的，跨模块 |
| UI | **按业务域+设备聚合**（推荐） | 同一业务域+设备的视觉风格一致，跨模块 |
| 技术方案 | **按技术实现边界聚合**（服务/模块/技术域） | 技术方案的边界是代码的物理边界，不是 PRD 的业务边界 |

PRD（业务视角）与技术方案（实现视角）是**多对多映射**，通过功能点编号建立关联。详细的目录结构示例、完整的项目目录树、以及编号规范参见 `references/workflow/product-organization.md`。
...
| 场景 | 必须执行的操作 | 红线 |
|------|--------------|------|
| **修改上游文档** | 1. 分层扫描下游（`python3 "$HOME/.agents/skills/e2e-solution-guard/scripts/doc-audit.py" --scan-downstream`）<br>2. 列出影响范围并按优先级排序<br>3. 询问用户是否级联修改<br>4. **用户确认** → 同步修改所有下游<br>5. **用户拒绝** → 标注 `[待同步]` | **禁止**不同步也不标注就留下不一致 |
| **修改下游产物** | 1. 读取所有上游文档<br>2. 逐条比对是否在上游定义范围内<br>3. **未超出** → 直接修改<br>4. **超出** → **立即停止**，先改上游 | **禁止**擅自扩展上游定义 |
